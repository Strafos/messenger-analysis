...
def message_freq(messages, participant):
    ...
    print("Gap\tZaibo Count\t%s Count\t%s %%\t %s %%" % (participant, MY_NAME, participant))
...
def average_response_time(message_json):
    ...
    prev_msg_t = datetime.datetime.fromtimestamp(first_message["timestamp"])
    prev_sender = first_message["sender_name"]
    for message in reversed(messages):
        curr_msg_t = datetime.datetime.fromtimestamp(message["timestamp"])
